...
+
+	if(chip_info->address32 == ADDRESS_LO3_HI4_BYTE || chip_info->address32 == ADDRESS_DIE_LO3_HI4_BYTE || chip_info->address32 == ADDRESS_4BYTE)
+	{
+		if(chip_info->address32 == ADDRESS_DIE_LO3_HI4_BYTE)
+		{
+			spi_generic_select_die(bank, 0,ctrl_drv);
+		}
+		if( exit_4byte_addr_mode(bank, ctrl_drv) < 0 )
+		{
+			printk ("Unable to exit 4 byte address mode\n");
+		}
+	}
